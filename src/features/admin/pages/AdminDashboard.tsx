...
export function AdminDashboard() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: contentData?.courses?.length || 0,
      icon: FileCheck,
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage users, approve content, and monitor platform health.
            </p>
          </div>
          <div className="flex gap-4">
            {stats.map((stat, i) => (
              <div
                key={i}
                className="bg-card border border-white/5 p-4 rounded-xl flex items-center gap-4 min-w-[180px] shadow-sm"
              >
        ...
        <Tabs defaultValue="users" className="space-y-6">
          <TabsList className="bg-muted/50 p-1">
            ...
            <TabsTrigger value="content" className="gap-2">
              ...
              {Number(contentData?.courses?.length) > 0 && (
