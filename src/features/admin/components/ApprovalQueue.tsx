...
export function ApprovalQueue({ courses }: ApprovalQueueProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {courses.map((course) => (
        <Card key={course.id} className="overflow-hidden flex flex-col border-none shadow-md ring-1 ring-white/10 bg-card/50">
          <div className="aspect-video relative overflow-hidden">
            ...
            <div className="absolute top-2 right-2">
              <Badge className={cn(
                ...
                course.status === 'PENDING_DELETE' ? "bg-red-500" : "bg-yellow-500"
              )}>
                {course.status === 'PENDING_DELETE' ? 'DELETE REQUEST' : 'NEW/UPDATED'}
              </Badge>
            </div>
          </div>
          <CardHeader className="p-4">
            <CardTitle className="text-lg line-clamp-1">{course.courseName}</CardTitle>
