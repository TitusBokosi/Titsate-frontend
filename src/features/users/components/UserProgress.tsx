...
export function UserProgress({ userId }: { userId: string }) {
  const { data: progressRes, isLoading: isLoadingProgress } =
    useUserProgress(userId);
  const { data: enrollmentRes, isLoading: isLoadingEnrollments } =
  ...
  const courseProgressData = enrollments.map((enrollment: any) => {
    ...
    const topicsWithProgress = topics.map((topic: any) => {
      ...
      const completedTopicLessons = lessons.filter((l: any) =>
        completedLessonIds.has(l.id),
      );
      const isCompleted =
        lessons.length > 0 && completedTopicLessons.length === lessons.length;
      const progressPercent =
        lessons.length > 0
          ? Math.round((completedTopicLessons.length / lessons.length) * 100)
          : 0;
      ...
      {courseProgressData.length === 0 ? (
        ...
        <div className="space-y-8 md:space-y-12 pb-20">
          {courseProgressData.map((course: any) => (
            <Card
              key={course.id}
              className="overflow-hidden border-none shadow-xl bg-secondary/5 ring-1 ring-white/5"
            >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center sm:justify-start gap-2 text-muted-foreground text-xs md:text-sm font-medium">
                        <span className="flex items-center gap-1">
                          ...
                          {course.totalTopicsCount} topics completed
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-center lg:items-end gap-2 w-full lg:w-auto lg:min-w-[200px]">
                    ...
                    <Progress
                      ...
                      className="h-2 md:h-2.5 w-full bg-primary/10"
                    />
                  </div>
                </div>
              </CardHeader>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:gap-4">
                    {course.topics.map((topic: any) => (
                      <Accordion key={topic.id} className="w-full">
                        <AccordionItem
                          value={topic.id}
                          className="border rounded-xl px-3 md:px-4 bg-background/40 hover:bg-background/60 transition-colors border-white/5 overflow-hidden"
                        >
                                ...
                                <div className="flex items-center justify-between mb-1.5 gap-2">
                                  <span
                                    className={cn(
                                      ...
                                      topic.isCompleted && 'text-green-500',
                                    )}
                                  >
                                    {topic.topicName}
                                  </span>
                    ...
                    >
                      <div className="flex flex-col md:flex-row items-center md:items-center justify-between gap-6 text-center md:text-left">
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                          ...
                          <div>
                            <h5 className="font-bold text-base md:text-lg mb-1">
                ...
                <CardFooter className="bg-secondary/5 border-t border-white/5 p-4 md:p-6 flex justify-center md:justify-end">
                  <Link
                    ...
                    className={cn(
                      ...
                      'w-full sm:w-auto gap-2 font-bold group shadow-lg shadow-primary/10',
                    )}
                  >
                    Continue Learning
